...
if (isset($input['idUsuario'], $input['nombreUsuario'], $input['motivoEnvio'], $input['nombreBolsa'], $input['mensajes'])) {
  if (isset($input['mensajes']['metodoEnvio'], $input['mensajes']['tipoMensaje'], $input['mensajes']['rawMensaje'], $input['mensajes']['mensajes'])) {
    if ($input['mensajes']['tipoMensaje'] == 'individual' || $input["mensajes"]['tipoMensaje'] == 'precargado') {
      $json = $api->insertSendMessageIndividualOrPrecargado(
        $input['idUsuario'],
        $input['nombreUsuario'],
        $fechaHoraRegistro,
        $input['motivoEnvio'],
        ...
        $input['mensajes']['valorMensajeIndividual']
      );
    } else {
